...
| `scripts/` | Pipeline CLI (`analyze`, `render`, `devlab`, `pr`, `quality-gate`, `validate-lottie`, `skill-doctor`, `report`) |
...
| `schemas/` | Task, report, artifact-manifest and handoff JSON Schemas |
| `references/` | Progressive-disclosure contracts for reporting and runtime capability |
...
The project-specific value remains in context binding, Motion Spec, asset provenance, runtime evidence, quality policy, review memory and handoff artifacts. Do not reimplement Lottie/Rive runtimes, MCP transport or GitHub authentication inside this Skill.
