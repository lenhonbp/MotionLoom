...
3c. **Ingest generation artifacts** — before using output from an internal skill or external provider, bind it with a `generation-receipt`, `control-track` and `export-manifest`. Run `motionloom artifact-intake intake --root <project-root> --registry artifact-adapter-registry.json --receipt <receipt> --controls <controls> --export-manifest <export> --json`. The core path makes no provider call and stores no secret; adapters are evidence descriptors. `internal-imagegen` is scaffold-only metadata for an Agent-managed ImageGen run, not proof of runtime readiness, artist authority or approval.
