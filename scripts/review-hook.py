...
def paths(task_dir: Path, task: dict) -> tuple[Path, Path, Path, Path]:
    ...
    if not source_path.is_file() or ROOT / "src" / "output" not in source_path.parents:
        raise ValueError("manifest.file must point to a source inside this repo's scene output")
...
def prepare(args: argparse.Namespace) -> int:
    ...
    url = f"{base}/?{urlencode({'scene': scene, 'task_id': task['task_id'], 'candidate_id': cid})}"
    candidate = {
        "schema_version": "1.0",
        "candidate_id": cid,
    ...
    report.setdefault("browser_review", []).append({"candidate_id": cid, "decision": "pending", "evidence": ["browser-review.json"], "next_action": "Open the internal Dev Lab URL and ask the user to review."})
    report.setdefault("next_agent", []).append({"id": "browser-review", "summary": "Open the exact candidate in the internal Dev Lab browser.", "status": "pending", "agent": "browser-review-agent", "skill": "browser-review", "evidence_needed": ["review.json"], "next_action": "Ask user to approve or request changes."})
    ...
    subprocess.run(["bash", str(ROOT / "scripts/devlab.sh"), scene, "--prepare-only"], check=True, capture_output=True, text=True)
...
def validate(args: argparse.Namespace) -> int:
    ...
    scene_dir, source_path, render_meta, context_path = paths(task_dir, task)
    spec = read_json(scene_dir / "motion-spec.json")
    expected = candidate_id(task["task_id"], task["scene"], (spec.get("context_binding") or {}).get("context_sha256", ""), sha256(source_path), sha256(render_meta))
    errors = []
    ...
    if candidate.get("context_sha256") != (spec.get("context_binding") or {}).get("context_sha256"):
